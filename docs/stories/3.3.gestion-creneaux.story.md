...
- [x] Task 2: Sélection de patient (AC: 2)
  - [x] Créer un composant `PatientSelect` avec recherche
  - [x] Utiliser Shadcn Combobox
  - [x] Permettre la recherche par nom/email
  - [x] Ajouter un bouton "Nouveau patient" qui ouvre le modal de création de patient existant (`CreatePatientModal` / `components/patients/create-patient-modal.tsx`), puis rafraîchir la liste et pré-sélectionner le patient créé
- [x] Task 3: Schéma de validation Zod (AC: 3)
  - [x] Créer `lib/validations/appointment.ts`
  - [x] Définir `appointmentSchema` (côté formulaire: startTime + duration; la server action convertit en startTime/endTime pour Prisma):
...
- **Tests (ajoutés 2026-05-16):**
  - `tests/unit/validations/appointment.test.ts` — 10 tests (3.3-UNIT-001 à 3.3-UNIT-003 + cas de bord) ✅
  - `tests/integration/calendar/appointment-actions.test.ts` — 19 tests (3.3-INT-001 à 3.3-INT-012 + auth/edge cases) ✅
...
- [x] Implémenter les tests d’intégration pour checkConflict, createAppointment, updateAppointment, deleteAppointment, updateAppointmentStatus (voir 3.3-INT-001 à 3.3-INT-012) — 19 tests verts dans `tests/integration/calendar/appointment-actions.test.ts` (2026-05-16, Dev)
- [x] Implémenter les tests unitaires pour appointmentSchema (3.3-UNIT-001 à 3.3-UNIT-003) — 10 tests verts dans `tests/unit/validations/appointment.test.ts` (2026-05-16, Dev)
- [ ] À terme : scénarios E2E 3.3-E2E-001 à 3.3-E2E-006 — externalisés dans la **Story 3.6 (dette technique runner E2E calendrier)**
...
|------|---------|-------------|--------|
| 2026-05-16 | 1.3 | Implémentation tests unit (10) + integration (19) couvrant 3.3-UNIT-001→003 et 3.3-INT-001→012 ; cochage subtasks Task 2/3 ; E2E externalisés en Story 3.6. **Demande revue QA pour passage du gate CONCERNS → PASS** | Dev (James) |
