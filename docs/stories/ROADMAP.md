...
| 13 | **13.1 CI/CD GitHub Actions (Done)** · **13.2 déploiement Vercel (Done — config CD reproductible : `postinstall` Prisma corrigeant le build Vercel, région cdg1 UE, validation env prod testable, runbook ; exécution réelle = ops/humaine OPS-002)** · **13.3 monitoring Sentry (Done — `@sentry/nextjs` ^10 gated/no-op sans DSN, scrubbing RGPD pur+testé `sendDefaultPii:false`+`beforeSend`, Replay off, `withSentryConfig` source maps gated token ; activation réelle = ops/humaine)** · 13.4 backups DB · **13.5 rate-limiting** | DevOps / Sécu infra |
...
| 2026-06-26 | 2.3 | Story 13.3 (**monitoring des erreurs Sentry — gated & RGPD-safe**) → Done (gate QA PASS, score 90 ; suite 93 fichiers / 819 tests verte ; `next build` vert Next 16 + Turbopack). `@sentry/nextjs` ^10 **désactivé par défaut** (no-op sans DSN, pattern SMS/chiffrement). Logique pure testée `lib/observability/sentry.ts` : gating, sampling, et **expurgation RGPD `scrubSentryEvent`** (`sendDefaultPii:false` + `beforeSend` retirant user/cookies/corps/en-têtes d'auth, redactant les tokens d'URL ; Session Replay off). Wiring App Router (instrumentation + onRequestError, configs server/edge/client), `withSentryConfig` (source maps gated sur `SENTRY_AUTH_TOKEN`). **Aucun compte/DSN créé, aucun événement émis, aucun secret committé.** Résiduels : `RGPD-133` (mitigé), `DEPS-133` (26 vulnérabilités transitives à auditer), `OPS-133` (activation réelle = ops/humaine). | SM / Dev / QA (boucle BMAD) |
